...
// Get total count of cleared penalties
$totalBooks = getClearedPenaltiesCount();
$totalPages = ceil($totalBooks / $perPage);
$offset = ($page - 1) * $perPage;

// Get cleared penalties with pagination (user_id = null for all users, limit, offset)
$clearedPenalties = getClearedPenalties(null, $perPage, $offset);

// Debug: Check if data is retrieved
echo "<!-- DEBUG: Found " . count($clearedPenalties) . " cleared penalty records -->";
if (count($clearedPenalties) > 0) {
    echo "<!-- DEBUG: First record: " . htmlspecialchars($clearedPenalties[0]['title']) . " -->";
}
?>
    ...
    <div class="table-responsive">
        <table class="table table-striped table-hover">
            ...
            <tbody id="penalty-table-body">
                <?php foreach($clearedPenalties as $penalty): ?>
                <tr>
                    <td><?php echo htmlspecialchars($penalty['title']); ?></td>
                    <td><?php echo htmlspecialchars($penalty['borrower_name']); ?></td>
                    <td><?php echo date('M d, Y', strtotime($penalty['borrow_date'])); ?></td>
                    <td><?php echo date('M d, Y', strtotime($penalty['return_date'])); ?></td>
                    <td><?php echo htmlspecialchars($penalty['days_overdue']); ?> days</td>
                    <td>₱<?php echo number_format($penalty['penalty_amount'], 2); ?></td>
                    <td><?php echo htmlspecialchars($penalty['cleared_by_name']); ?></td>
                    <td><?php echo date('M d, Y', strtotime($penalty['cleared_at'])); ?></td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
